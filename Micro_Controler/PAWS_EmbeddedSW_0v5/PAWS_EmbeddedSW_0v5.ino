...
bool getCharacteristicBool(BLECharacteristic* pChar) {
  ...
  // Accept binary 0x01
  if ((uint8_t)val[0] == 1) return true;

  // Also accept ASCII '1' from some BLE apps
...
class SettingWriteCallback : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* pChar) override {
    ...
    // Always print this so we know the callback is actually firing
    Serial.println("[BLE] Write callback fired");

    String uuid = pChar->getUUID().toString().c_str();
    uuid.toUpperCase();

    Serial.print("[BLE] Written UUID: ");
    Serial.println(uuid);
    ...
    if (uuid == BLE_CHAR_CLOCK_UUID) {
      ...
      Serial.print("[BLE] Clock set to: ");
      Serial.println(Settings[6].Value);
    }
      ...
      Serial.print("[BLE] Backlight timer set to: ");
      Serial.println(Settings[7].Value);
    }

    else if (uuid == BLE_CHAR_LCD_LOCK_UUID) {
      bool lock = getCharacteristicBool(pChar);
      Settings[9].Value = lock ? 1.0f : 0.0f;
      Serial.print("[BLE] LCD Lock set to: ");
      Serial.println(lock ? "ON" : "OFF");
    }

    else if (uuid == BLE_CHAR_DEBUG_UUID) {
      bool dbg = getCharacteristicBool(pChar);
      Settings[8].Value = dbg ? 1.0f : 0.0f;

      // Do not use DEBUG_PRINT here, because debug may have been off
      Serial.print("[BLE] Debug set to: ");
      Serial.println(dbg ? "ON" : "OFF");

      Serial.print("[BLE] S_DEBUG now = ");
      Serial.println(S_DEBUG);
    }
    ...
    // ---------------- Controls Service ----------------
      ...
      Serial.println("[BLE] Unknown UUID written");
    }
...
void readSensors() {
  ...
  //  DEBUG OUTPUT
  // ==========================================================
  DEBUG_PRINT("[SENSOR] T=%.1fC H=%d%% Wind=%s Rain=%.2f/m Light=%d\n",
  tempDegC, humidPercent, windLevelStr[windLevel], rainTipsPerMin, lightLevel);

  DEBUG_PRINT("[SET] Tsp=%.1f Thys=%.1f Hsp=%.0f Hhys=%.0f Wmax=%.1f Lmin=%.0f\n",
  S_TEMP_SP, S_TEMP_HYS, S_HUMID_SP, S_HUMID_HYS, S_WIND_MAX, S_LIGHT_MIN);

  DEBUG_PRINT("[CTRL] WinMode=%s WinState=%s LightMode=%s LightState=%s\n",
  (C_WIN_OVR >= 1.0f ? "FORCE_OPEN" : "AUTO"),
  (windowIsOpen ? "OPEN" : "CLOSED"),
  (C_LIGHT_OVR >= 1.0f ? "FORCE_ON" : "AUTO"),
  (digitalRead(PIN_LED_LIGHT) == HIGH ? "ON" : "OFF"));
...
void autoControl() {
  ...
  if (C_WIN_OVR >= 1.0f) {
    // Manual override:
    // if override is ON, force window OPEN regardless of environment
    ...
    // Keep automatic result visible in Window Set
    C_WIN_SET = targetWindowOpen ? 1.0f : 0.0f;
  }
  ...
  if (C_LIGHT_OVR >= 1.0f) {
    // Manual override:
    // if override is ON, force light ON regardless of light sensor
    lightOn = true;
  } else {
    // Automatic environmental control
    ...
    // Keep automatic result visible in Light Set
    C_LIGHT_SET = lightOn ? 1.0f : 0.0f;
  }
